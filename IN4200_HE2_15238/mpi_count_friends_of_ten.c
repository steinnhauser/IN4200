int MPI_count_friends_of_ten(int M, int N, int** v)
{
    ...
    if (verbose) {
        printf("Counting 'triple-friends of 10' in parallel for matrix:\n");
        printf("[\n");   
        for (int i = 0; i < M; i++)
        {
            printf("   [");
    ...
    // Initialize parallel region, where all cores have access to the v array.
    #pragma omp parallel shared(v, M, N, no, verbose)
    {     
        // Initialize booleans which describe which triple friends combinations to examine.
        int bool1, bool2, bool34, rank = omp_get_thread_num();

        #pragma omp for reduction(+:no) private(bool1, bool2, bool34)
        for (int m = 0; m < M; m++)
        {
            for (int n = 0; n < N; n++)
            {
